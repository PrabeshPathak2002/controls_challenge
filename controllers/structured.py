...
class Controller(BaseController):
  """preview_roll + speed schedule, dual roll FF, tanh FF, anti-windup."""
  ...
  def __init__(
    ...
    i=0.100,
    d=-0.053,
    # Tuned on 120 clips; holdout-gated vs preview_roll (~61.5 vs ~64.3).
    preview_gain=0.5,
    preview_steps=4,
    roll_gain=0.53,
  ...
  def _soft(self, x: float) -> float:
    # tanh soft-clip: linear near 0, saturates for large |x|
  ...
  def update(self, target_lataccel, current_lataccel, state, future_plan):
    ...
    # provisional PID using current integral (anti-windup applied after)
    ...
    # Schedule feedback + FF together so high-speed is calmer.
    command = speed_scale * (pid + preview_ff + roll_ff + future_roll_ff)

    # Conditional integration + hard clamp (anti-windup)
